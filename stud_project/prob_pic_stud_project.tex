...
\begin{axis}[no markers, domain=0:10, samples=100, xlabel={x}, enlargelimits=false,
height=7cm, width=10cm, clip=false, axis on top, ylabel=$f(x)$,
grid = major]
...
\begin{axis}[no markers, domain=0:10, samples=100, xlabel={x}, enlargelimits=false, ylabel=$F(x)$,
...
\pgfmathdeclarefunction{pareto}{2}{\pgfmathparse{( #1 * #2 ^ ( #1 ) / ( x ^ (#1 + 1)))}}

\begin{tikzpicture}
\begin{axis}[no markers, domain=0:10, samples=100, xlabel={x}, ylabel={$f(x)$}, legend pos=south east,  enlargelimits=false, ytick = {0, 0.5, ..., 3}, 
...
height=8cm, width=10cm, clip=false, axis on top, legend pos=south east, 
grid = major, ylabel = $F(x)$]
\addplot [domain=1:10, color = blue] {paretof(1, 1)} \closedcycle;
\addlegendentry{$k = 1, x_m = 1$};
\addplot [domain=2:10, color = black] {paretof(1, 2)} \closedcycle;
\addlegendentry{$k = 1, x_m = 2$};
...
\begin{tikzpicture}[
    ...
    \begin{axis}
    [xlabel = $x$, height=10cm, width=12cm, ylabel=$f(x)$
...
На данной картинке изображена динамика среднего значения  выпавшего кубика в зависимости от количества подбрасываний.

Как можно наблюдать, при достаточно большом количестве подбрасываний среднее значение стремтися к математическому ожиданию.
